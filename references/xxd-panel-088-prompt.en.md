# XXD Panel 088 | Runtime Adapter (English)

This is not a second aesthetic prompt. Read `references/original-prompt/zh-CN.md` in full before generation; it is the sole creative and aesthetic authority for Panel 088. Its first Markdown heading is an internal archival label and does not enter the image request; use the entire remaining source-brief body verbatim. This file only appends the current user's runtime variables.
