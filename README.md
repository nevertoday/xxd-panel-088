<p align="center"><img src="./assets/banner.svg" alt="XXD Panel 088 项目横幅" width="1200"></p>
...
`sample-01`–`sample-04` 保留给未来可核验的原始发布样张。以上八张包括四张 16:9 横版左右样张和四张 3:4 竖版上下样张，均由 Panel 088 依据自己的原始提示词独立生成，不借用其他编号作品。样张使用英文智能配字。
...
```text
分析主体、轮廓、结构、姿态与叙事关系 → 提取少量有意义的词、字符与点阵 → 以文字密度、字号、方向、旋转、字距和重叠重建外轮廓 → 用聚合暗部与稀疏亮部塑造内部 → 让点阵表达圆形、眼睛、灯光、轮胎等中心结构 → 在留白中保留少量碎片文字与残迹
```

## 成品中最容易识别的风格特点

- 文字不是附加说明，而是主体的骨骼、轮廓、阴影和纹理；视觉形态优先于完整阅读，但不能退化为随机字符噪声。
- 关键外轮廓保持清晰，内部允许密集、拉开、竖排、倒置、旋转、断裂和单字符停顿。
- 圆形、眼睛、灯光、轮胎、花心、窗洞和太阳等中心结构可使用同心圆点阵、放射文字或环形字符。
- 配色以深色文字墨色、1–2种原图衍生强调色和极浅近白背景为主。
- 拒绝传统 ASCII 字符画、普通文字云、随机代码感、赛博故障风、完整描边插画、商业字体海报和模板化排版。

原始审美约束与拒绝项只存在于[原始提示词](references/original-prompt/zh-CN.md)；Skill 与运行适配器只处理本次交付变量。 [Skill 工作流](SKILL.md) · [英文运行适配器](references/xxd-panel-088-prompt.en.md)
...
| **[xxd-panel-088](https://github.com/nevertoday/xxd-panel-088)** | 实验字体图像 · 解构排版 · 点阵轮廓 · 视觉诗 · 文字密度梯度 · 主动留白 |
...
| **士兵级** | `xxd-panel-NNN`（当前 001–088） | 每个编号只执行自己独立的原始提示词与审美，把将军派发的单个任务完成为成品。 |
