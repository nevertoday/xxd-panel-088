<p align="center"><img src="./assets/banner.svg" alt="XXD Panel 088 프로젝트 배너" width="1200"></p>
...
`sample-01`–`sample-04`는 검증 가능한 원본 공개 예시를 위해 남겨 둡니다. 위 여덟 작품은 16:9 가로형 좌우 구성 네 장과 3:4 세로형 상하 구성 네 장입니다. 모두 Panel 088이 자체 원본 프롬프트에 따라 독립적으로 생성했으며 다른 번호의 작품을 재사용하지 않았습니다. 예시 문구는 영어로 자동 생성했습니다.

<!-- xxd-human-intro:start -->
## 이 스타일이 해결하는 문제

완전한 외곽선은 피사체를 평범한 삽화로 만들고, 글자 구름은 무작위 노이즈만 남깁니다. 언어가 이미지 구조가 되려면 둘 다 부족합니다.

**Panel 088**은 완전한 단어·글자·한자·숫자·구두점·조각 텍스트와 점 배열 자체를 조형 재료로 사용합니다. 멀리서는 사진의 피사체가 선명하게 보이고, 가까이서는 윤곽·어두운 영역·무게와 질감이 밀도·크기·방향·회전·자간·겹침으로 구성된 문자임을 발견합니다.

### 이런 경우에 적합합니다

- 읽을 수 있는 단어·한 글자·구두점·점이 추가 제목이 아니라 피사체 자체가 되기를 원할 때
- 문자를 어두운 부분에 모으고 밝은 부분에는 벌려 놓으면서 핵심 외곽을 선명하게 유지하는 해체적 구성이 필요할 때
- 문자 밀도 그라데이션, 양과 음의 형태, 방향의 긴장감과 넓은 여백으로 시각시를 만들고 무작위 문자 노이즈는 피하고 싶을 때
- 상하·좌우·디자인 전용·여러 비율·벽지·폴더 일괄 처리

### 빠른 시작

> 이 이미지를 XXD Panel 088로 처리하고 가장 알맞은 구도와 크기를 먼저 추천해 주세요.
...
**특징:** 글자의 이미지화 · 해체 타이포그래피 · 점 윤곽 · 시각시 · 밀도 그라데이션 · 능동적 여백
...
| 확인할 점 | 이 스타일의 답 |
|---|---|
| **결과물** | 멀리서는 피사체, 가까이서는 문자와 점으로 이루어진 실험적 이미지 포스터 |
| **대표 특징** | 선명한 외곽, 문자 밀도 그라데이션, 회전·단절 문자, 점 구조, 능동적 여백 |
| **입력 존중 방식** | 피사체의 정체성·동작·환경·은유에서 글자를 길어 올려 윤곽·뼈대·그림자·질감으로 사용함 |
...
```text
피사체·윤곽·구조·자세·서사적 관계 분석 → 의미 있는 단어·문자·구두점·점 추출 → 밀도·크기·방향·회전·자간·겹침으로 외곽 재구성 → 조밀한 글자로 어두운 부분과 무게를, 성긴 글자로 밝은 부분과 공기를 형성 → 중심 구조에 점 윤곽과 방사형 문자 사용 → 소량의 조각 문자를 여백으로 흘려보냄
```

## 완성작의 특징

- 글자는 추가 설명이 아니라 피사체의 뼈대·윤곽·그림자·질감입니다. 시각적 형태를 우선하되 무작위 문자 노이즈로 만들지 않습니다.
- 핵심 외곽은 명확하게 유지하고 내부는 밀집·확산·세로쓰기·반전·회전·단절·한 글자만 남기는 구성을 허용합니다.
- 원·눈·조명·타이어·꽃술·창의 개구부·태양 같은 중심 구조에는 동심원 점 배열, 방사형 문자 또는 원형 문자 배열을 사용할 수 있습니다.
- 짙은 문자 잉크, 원본에서 파생한 강조색 1–2개, 매우 옅은 미색 배경을 사용합니다.
- 전통적인 ASCII 문자 그림, 평범한 워드 클라우드, 무작위 코드 느낌, 사이버 글리치 스타일, 완전한 외곽선 삽화, 상업적 타이포그래피 포스터와 틀에 박힌 레이아웃을 피합니다.

미적 제약은 [원본 프롬프트](references/original-prompt/zh-CN.md)에만 있습니다. Skill과 실행 어댑터는 납품 변수만 처리합니다. [Skill](SKILL.md) · [영문 실행 어댑터](references/xxd-panel-088-prompt.en.md)
...
이미지 폴더를 직접 전달하면 이 병사 Skill이 같은 Panel 088의 원본 미학으로 일괄 처리합니다. 일반 래스터 형식을 기본적으로 재귀 탐색하고 수량을 먼저 알린 뒤 모드·크기·텍스트·언어를 한 번만 확인합니다. 각 이미지는 내용이나 문구가 섞이지 않도록 독립 생성하며 전체 배치는 하나의 평면 작업 디렉터리만 사용합니다. 개별 실패는 기록하고 이후 이미지를 조용히 누락하지 않습니다.
...
/xxd-panel-088 photo.jpg --mode top-bottom --size auto --text prompt --locale ko-KR

# 좌우 비교: 3:2 가로형, 글자 없음
/xxd-panel-088 photo.jpg --mode left-right --size 3:2 --text none

# 디자인 전용: 9:16 휴대폰 세로형, 한국어 문구
/xxd-panel-088 photo.jpg --mode design-only --size 9:16 --text prompt --locale ko-KR
...
/xxd-panel-088 photo.jpg --mode design-only --size 3:4 --text prompt --locale ko-KR

# 정확한 문구를 재작성·번역·제목 추가 없이 사용
/xxd-panel-088 photo.jpg --mode design-only --size 3:4 --copy "빛을 이곳에 남겨 두기" --locale ko-KR

# 2개 모드 × 3개 크기로 독립적인 완성 구도 6개 생성
/xxd-panel-088 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale ko-KR

# 폴더 전체에 Panel 088 적용, 공통 설정은 한 번만 확정
/xxd-panel-088 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale ko-KR
...
Claude Code 사용자는 같은 폴더를 다음 위치에 연결할 수 있습니다: `~/.claude/skills/xxd-panel-088`. 설치 후 에이전트 세션을 다시 시작하세요.
...
전체 사양: [Skill 워크플로](SKILL.md) · [원본 스타일 자료](references/original-prompt/zh-CN.md) · [영문 런타임 어댑터](references/xxd-panel-088-prompt.en.md) · [중문 런타임 어댑터](references/xxd-panel-088-prompt.zh-CN.md)
...
| **[xxd-panel-088](https://github.com/nevertoday/xxd-panel-088)** | 글자의 이미지화 · 해체 타이포그래피 · 점 윤곽 · 시각시 · 밀도 그라데이션 · 능동적 여백 |
...
| **병사급** | `xxd-panel-NNN` (현재 001–088) | 각 번호가 고유한 원본 프롬프트와 미학만 실행해 장군이 배정한 하나의 작업을 완성합니다. |
