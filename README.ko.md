...
예시 영역은 예약되어 있습니다. 검증 가능한 원작 링크나 088 원본 프롬프트와 직접 대조한 확장 예시가 준비되면 이곳에 먼저 표시하며, 다른 번호의 이미지나 깨진 자리표시자로 대신하지 않습니다.
...
**Panel 088**은 단어·문자·숫자·구두점·조각 텍스트와 점 자체를 조형 재료로 사용합니다. 멀리서는 사진의 주제가 보이고 가까이서는 윤곽·어두운 영역·무게와 질감이 문자로 이루어졌음을 발견합니다.

### 이런 경우에 적합합니다

- 글자가 제목이 아니라 피사체의 뼈대·윤곽·그림자·질감이 되기를 원할 때
- 문자 밀도, 크기, 방향, 회전, 자간과 겹침으로 해체적 구성을 만들고 싶을 때
- 시각적 형태를 우선하면서도 무작위 문자 노이즈는 피하고 싶을 때
...
| **대표 특징** | 선명한 외곽, 문자 밀도 그라데이션, 회전·단절 문자, 점 윤곽, 능동적 여백 |
| **입력 존중 방식** | 사진의 정체성·행동·환경·은유에서 글자를 길어 올려 이미지 구조로 사용함 |
...
```text
주제·윤곽·구조·자세·관계 분석 → 의미 있는 단어·문자·구두점·점 추출 → 밀도·크기·방향·회전·자간·겹침으로 외곽 재구성 → 조밀한 글자로 어두운 무게, 성긴 글자로 밝은 공기 형성 → 점 윤곽과 방사형 문자를 초점에 사용 → 조각 문자를 여백으로 흘려보냄
```

## 완성작의 특징

- 글자는 추가 설명이 아니라 피사체의 뼈대·윤곽·그림자·질감입니다.
- 핵심 외곽은 명확하게 유지하고 내부는 밀집·확산·회전·반전·단절을 허용합니다.
- 눈·빛·바퀴·창·태양 같은 중심 구조에는 동심 점 배열이나 방사형 문자를 사용할 수 있습니다.
- 깊은 문자 잉크, 원본에서 파생한 1~2색 강조색, 거의 흰 배경을 사용합니다.
- ASCII 아트, 일반 문자 구름, 무작위 코드, 사이버 글리치, 상업용 폰트 포스터를 피합니다.
