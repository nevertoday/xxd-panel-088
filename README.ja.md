...
作例枠は確保済みです。検証可能な原作リンク、または 088 原文プロンプトに直接照合した追加作例が揃い次第ここで優先表示し、別番号の画像や壊れたプレースホルダーで代用しません。
...
**Panel 088** は単語、文字、数字、句読点、断片文、点を造形素材にします。遠くからは写真の主体が明確に見え、近づくと輪郭、影、重量、質感が文字で構成されていることが分かります。

### 向いている用途

- 文字を付加タイトルではなく、主体の骨格、輪郭、影、質感にしたい場合
- 密度、サイズ、方向、回転、字間、重なり、断裂による解構的排版が必要な場合
- 視覚形態を優先しながら、無作為な文字ノイズは避けたい場合
...
**特徴：** 文字の画像化 · 解構タイポグラフィ · 点描輪郭 · 視覚詩 · 密度勾配 · 能動的余白
...
| **識別しやすい特徴** | 明確な輪郭、文字密度勾配、回転・断裂文字、点描構造、能動的余白 |
| **入力の尊重** | 写真の同一性、動作、環境、記憶、比喩から言葉を生み、画像構造へ変換する |
...
```text
主体・輪郭・構造・姿勢・関係を分析 → 意味のある単語・文字・句読点・点を抽出 → 密度・サイズ・方向・回転・字間・重なりで外輪郭を再構成 → 密集文字を暗部、疎な文字を明部にする → 焦点構造を点描や放射文字で表す → 断片を能動的余白へ解放する
```

## 完成作の特徴

- 文字は付加説明ではなく、主体の骨格、輪郭、影、質感になる。
- 識別に必要な外輪郭を明確にし、内部では文字の密集、拡散、回転、倒置、断裂を許す。
- 目、光、車輪、花芯、窓、太陽には同心点描、放射文字、環状文字を使える。
- 深い文字インク、原写真由来の1〜2アクセント、ほぼ白い背景で色を構成する。
- ASCII アート、ワードクラウド、ランダムコード、サイバーグリッチ、完全描線、テンプレート排版を避ける。
