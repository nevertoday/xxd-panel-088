<p align="center"><img src="./assets/banner.svg" alt="XXD Panel 088 プロジェクトバナー" width="1200"></p>
...
`sample-01`–`sample-04` は、検証可能なオリジナル公開作例のために確保しています。上の8点は、16:9の横長・左右構成4点と、3:4の縦長・上下構成4点です。すべて Panel 088 が自身の原文プロンプトに基づいて個別に生成したもので、別番号の作品は流用していません。作例のコピーは英語で自動生成しています。

<!-- xxd-human-intro:start -->
## このスタイルが解決すること

完全な輪郭線は主体を普通の挿絵にし、ワードクラウドは無作為なノイズだけを残します。どちらも言語を画像構造にはしません。

**Panel 088** は、完全な単語、文字、漢字、数字、句読点、テキスト断片、ドットマトリクスを造形素材にします。遠くからは写真の主体が明確に見え、近づくと輪郭、暗部、重量、質感が、密度、サイズ、方向、回転、字間、重なりによって組まれた文字だと分かります。

### 向いている用途

- 読める単語、1文字、句読点、ドットそのものを、付加タイトルではなく主体にしたい場合
- 文字を暗部へ密集させ、明部では間隔を広げつつ、重要な外輪郭を明確に保つ脱構築的な組版が必要な場合
- 文字密度の勾配、正負の形、方向の緊張感、広い余白で視覚詩を組み立て、無作為な文字ノイズを避けたい場合
- 上下・左右・デザインのみ・複数比率・壁紙・フォルダ一括処理

### すぐ試す

> この画像を XXD Panel 088 で処理し、最適な構図とサイズを先に提案してください。
...
中国語ファイルが逐語の原文であり、実行時の唯一の創作上の基準です。他4言語は忠実な読解訳で、画像生成プロンプトを書き換えません。

**特徴：** 文字の画像化 · 脱構築的タイポグラフィ · 点描輪郭 · 視覚詩 · 密度勾配 · 能動的な余白
...
| 確認したいこと | このスタイルの答え |
|---|---|
| **得られるもの** | 遠くでは主体、近くでは文字と点のシステムとして読める実験ポスター |
| **識別しやすい特徴** | 明確な外輪郭、文字密度の勾配、回転・分断された文字、点描構造、能動的な余白 |
| **入力の尊重** | 主体の同一性、動作、環境、比喩から言葉を生み、輪郭、骨格、影、質感にする |
...
```text
主体・輪郭・構造・姿勢・物語上の関係を分析 → 意味のある単語・文字・句読点・ドットを抽出 → 密度・サイズ・方向・回転・字間・重なりで外輪郭を再構成 → 密集した文字で暗部と重量を、疎な文字で明部と空気を作る → 中心性のある構造を点描や放射文字で表す → 少量の断片を余白へ解放する
```

## 完成作の特徴

- 文字は付加説明ではなく、主体の骨格、輪郭、影、質感になる。視覚形態を優先しつつ、無作為な文字ノイズにはしない。
- 識別に必要な外輪郭を明確にし、内部では文字の密集、拡散、縦組み、反転、回転、分断、1文字だけの間を許す。
- 円、目、光、タイヤ、花芯、窓の開口部、太陽などには、同心円状のドット、放射文字、環状文字を使える。
- 濃色の文字インク、元画像由来のアクセントカラー1–2色、ごく淡いオフホワイトの背景で配色を構成する。
- 従来型の ASCII 文字絵、一般的なワードクラウド、無作為なコード感、サイバーグリッチ風、完全な輪郭線によるイラスト、商業的な文字ポスター、テンプレート化されたレイアウトを避ける。

美的制約は[原文プロンプト](references/original-prompt/zh-CN.md)だけにあります。Skill と実行アダプターは納品変数のみを扱います。 [Skill](SKILL.md) · [英語実行アダプター](references/xxd-panel-088-prompt.en.md)
...
画像フォルダーを直接渡すと、この兵士 Skill が同じ Panel 088 の原文美学で一括処理します。一般的なラスター形式を既定で再帰検索し、件数を報告してから、モード・サイズ・文字・言語を一度だけ確認します。各画像は内容や文言を混ぜず独立生成し、バッチ全体は一つのフラットなタスクフォルダーへ保存します。個別の失敗は記録し、後続画像を黙って省略しません。
...
/xxd-panel-088 photo.jpg --mode top-bottom --size auto --text prompt --locale ja-JP
...
/xxd-panel-088 photo.jpg --mode design-only --size 3:4 --text prompt --locale ja-JP

# 正確な文言を、書き換え・翻訳・タイトル追加なしで使用
/xxd-panel-088 photo.jpg --mode design-only --size 3:4 --copy "光をここに残す" --locale ja-JP

# 2モード × 3サイズで独立した完成構図を6枚生成
/xxd-panel-088 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale ja-JP

# フォルダー全体へ Panel 088 を適用し、共通設定は一度だけ解決
/xxd-panel-088 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale ja-JP
...
Claude Code では同じフォルダを次へリンクできます： `~/.claude/skills/xxd-panel-088`. インストール後に Agent セッションを再起動してください。
...
完全仕様: [Skill ワークフロー](SKILL.md) · [原始スタイル資料](references/original-prompt/zh-CN.md) · [英語ランタイムアダプター](references/xxd-panel-088-prompt.en.md) · [中国語ランタイムアダプター](references/xxd-panel-088-prompt.zh-CN.md)
...
| **[xxd-panel-088](https://github.com/nevertoday/xxd-panel-088)** | 文字の画像化 · 解構タイポグラフィ · 点描輪郭 · 視覚詩 · 密度勾配 · 能動的余白 |
...
| **兵士級** | `xxd-panel-NNN`（現在 001–088） | 各番号が固有の原文プロンプトと美学だけを実行し、将軍から渡された一つの仕事を完成させます。 |
